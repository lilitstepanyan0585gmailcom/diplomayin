...
class MALA_sampling(BOTorchOptimizer):
    ...
    def _run_single_chain(self, x0: torch.Tensor):
        ...
        for _ in range(self.n_burn_in_steps):
            ...
            burn_accepts += int(acc)
            burn_total += 1

        burn_acc_rate = burn_accepts / max(burn_total, 1)

        if self.adapt_step:
            if burn_acc_rate < self.target_acceptance - 0.10:
                step_size *= 0.85
            elif burn_acc_rate > self.target_acceptance + 0.10:
                step_size *= 1.05
        ...
        acc_rate = chain_accepts / max(chain_total, 1)
    ...
    def sample(self) -> torch.Tensor:
        ...
        for _ in range(self.n_restarts):
            ...
            if best_val > best_val_global:
                ...
                best_used_step = used_step

        self.step_size = 0.8 * self.step_size + 0.2 * best_used_step
        self.step_size = max(self.min_step_size, min(self.step_size, self.max_step_size))
